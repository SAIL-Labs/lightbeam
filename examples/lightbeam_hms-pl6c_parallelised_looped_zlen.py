...
#%%###########################################################################
### Calc Prop Modes for Lantern (does not depend on z_len)
...
#%%###########################################################################
### Initialise (fixed across z_len sweep)
...
## Worker globals that do not change with z_len
_w_modes      = modes
...
for z_len in z_len_list:
    print(f"\n{'='*60}")
    print(f"z_len = {z_len} um")
    print(f"{'='*60}")

    ## Check guided modes (does not depend on z_len, but skip if unexpected)
    if [tuple(m) for m in guided_modes] != expected_modes:
        print(f"Skipping z_len={z_len}: unexpected guided modes {guided_modes}")
        continue
