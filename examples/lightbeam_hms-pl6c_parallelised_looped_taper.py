...
## Mesh (does not depend on taper_ratio) ##
...
#%%###########################################################################
### Initialise (fixed across taper_ratio sweep)
...
## Worker globals that do not change with taper_ratio
_w_modes      = modes
...
#%%###########################################################################
### Sweep over taper_ratio

for taper_ratio in taper_ratio_list:
    print(f"\n{'='*60}")
    print(f"taper_ratio = {taper_ratio}")
    print(f"{'='*60}")

    ## Input radii depend on taper_ratio
    ...
    ## Guided modes depend on r_clad, so recalculate and check each iteration
    ...
    if [tuple(m) for m in guided_modes] != expected_modes:
        print(f"Skipping taper_ratio={taper_ratio}: unexpected guided modes {guided_modes}")
        continue
