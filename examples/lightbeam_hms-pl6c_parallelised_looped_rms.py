...
## coef_r_ms values to sweep
coef_r_ms_list = np.arange(1.25, 2.5, 0.25) # free variable
...
## Input radii that do not depend on coef_r_ms ##
r_core_wfs   = r_core_wfs_out   / taper_ratio
r_clad       = r_clad_out       / taper_ratio
core_spacing = core_spacing_out / taper_ratio

## Resolution ##
ds = 0.25 # [um]
dz = 2 # [um]

## Mesh (does not depend on coef_r_ms) ##
...
_mesh = RectMesh3D(xw, yw, z_len, ds, dz, num_PML)
x_grid, y_grid = _mesh.xg[num_PML:-num_PML, num_PML:-num_PML], \
                 _mesh.yg[num_PML:-num_PML, num_PML:-num_PML]

#%%###########################################################################
### Calc Prop Modes for Lantern (does not depend on coef_r_ms)

wave_number  = 2*np.pi/wl
norm_freq    = LPmodes.get_V(wave_number, r_clad, n_clad, n_cap)
...
#%%###########################################################################
### Initialise (fixed across coef_r_ms sweep)
...
## Worker globals that do not change with coef_r_ms
_w_modes      = modes
...
#%%###########################################################################
### Sweep over coef_r_ms

for coef_r_ms in coef_r_ms_list:
    print(f"\n{'='*60}")
    print(f"coef_r_ms = {coef_r_ms}")
    print(f"{'='*60}")

    ## Check guided modes (does not depend on coef_r_ms, but skip if unexpected)
    if [tuple(m) for m in guided_modes] != expected_modes:
        print(f"Skipping coef_r_ms={coef_r_ms}: unexpected guided modes {guided_modes}")
        continue

    ## r_core_ms depends on coef_r_ms
    r_core_ms_out = coef_r_ms * r_core_wfs_out
    r_core_ms     = r_core_ms_out / taper_ratio
    ...
    ## Update worker globals that change with coef_r_ms
    _w_pl6      = pl_6
    _w_f_suffix = f_suffix

    ## Run Parallelised Propagation
    ...
    for i in range(n_modes):
        ...
        cores_weighted = isolate_cores(
            ...
            core_locs_out, ds=ds, crop_size=80)

        weights_cores = isolate_cores(
            U_out_weights_array[i, num_PML:-num_PML, num_PML:-num_PML],
            core_locs_out, ds=ds, crop_size=80)

        u_lp01_probe_wfs = probe_field(cores_weighted, r_core_wfs_out,
                                       wl, n_core_wfs, n_clad, ds)
        u_lp01_probe_ms  = probe_field(cores_weighted, r_core_ms_out,
                                       wl, n_core_ms,  n_clad, ds)

        for j in range(n_cores):
            if j == 0:
                probe_j = normalize_weighted(u_lp01_probe_ms,  weights_cores[j], normval=1.0)
            else:
                probe_j = normalize_weighted(u_lp01_probe_wfs, weights_cores[j], normval=1.0)
