...
import numpy as np
from numpy import s_, arange, sqrt,\
    power, complex64 as c64
...
# combine the different remeshing options into a single func with a switch 
# argument
...
class RectMesh2D:
    ...
    def reinit(self, xw, yw):
        ...
        self.xa0 = np.linspace(-xw/2 - Nbc*dx, 
                               xw/2 + Nbc*dx, 
                               xres)
        self.ya0 = np.linspace(-yw/2 - Nbc*dy, 
                               yw/2 + Nbc*dy, 
                               yres)
        ...
        self.xa = self.xa0 = np.linspace(-xw/2 - Nbc*dx,
                                         xw/2 + Nbc*dx,
                                         xres)
        self.ya = self.ya0 = np.linspace(-yw/2 - Nbc*dy,
                                         yw/2 + Nbc*dy,
                                         yres)
    ...
    def update(self, rfacxa, rfacya):
        ...
        ne.evaluate("(a+b)/2",
                    local_dict={"a":self.xg[1:], "b":self.xg[:-1]}, 
                    out=xhg[1:-1])
        ne.evaluate("(a+b)/2",
                    local_dict={"a":self.yg[:,1:], "b":self.yg[:,:-1]},
                    out=yhg[:,1:-1])
    ...
    def get_weights(self):
        ...
        weights = ne.evaluate("(a-b)*(c-d)",
                              local_dict={"a":xhg[1:], "b":xhg[:-1], "c":yhg[:,1:], 
                                          "d":yhg[:,:-1]})
    ...
    ##########################################################################
    def resample(self, u, xa=None,
                 ya=None, newxa=None, newya=None):

        if xa is None or ya is None:
            out = RectBivariateSpline(self.xa_last, self.ya_last, u)\
                (self.xa, self.ya)
    ...
    ##########################################################################
    def resample_complex(self, u, xa=None,
                         ya=None, newxa=None, newya=None):
    ...
    def _compute_refinement_factor(self, u0, crit_val):
        ''' given some electric field u0, compute values that corresponds to 
            the degree of refinement required in the x and y subdivisions in 
            the field. larger refinement factors should imply more grid 
            refinement. 

            crit_val determines the normalization of this refinement factor 
            - this is left as an argument to allow the degree of mesh 
            subdivision to be controlled. note that the output refinement 
            factors, _rx and _ry, essentially will act as booleans. wherever 
            these factors are larger than 1, the grid will marked for subdivision.
        '''
        
        # the default scheme presented here sets the refinement factor to the 
        # geometric mean of field amplitude and field second derivative magnitude. 
        # convergence testing shows that this metric leads to faster convergence 
        # over using just field amplitude or second derivative magnitude alone. 
        # evidence for this is entirely empirical and comes from testing that 
        ...
        # field amps
        umaxx = np.sqrt(np.max(np.abs(u0), axis=1) \
                        * np.max(np.abs(xdif2), axis=1))
        umaxx = 0.5*(umaxx[1:] + umaxx[:-1])

        umaxy = np.sqrt(np.max(np.abs(u0), axis=0) \
                        * np.max(np.abs(ydif2), axis=0))
    ...
    def refine_by_two(self, u0, crit_val):
        ...
        ''' uses a hybrid approach where cells tagged are based on the product 
            of field amplitude and second derivative magnitude 
        '''
        ...
        return self.resample_complex(u0, xa_old, ya_old, 
                                     self.xa, self.ya)
    ...
    def refine_base(self, u0, ucrit):
        ''' 
            iteratively apply refine_by_two to fully subdivide the simulation 
            grid
        '''
...
class RectMesh3D:
    ...
    def __init__(self, xw, yw,
        ...
        self.xres, self.yres, self.zres = round(xw/ds) + 1 + 2*PML, \
            round(yw/ds) + 1 + 2*PML, \
                round(zw/dz) + 1

        self.xa = np.linspace(-xw/2 - PML*ds, 
                              xw/2 + PML*ds, 
                              self.xres)
        self.ya = np.linspace(-xw/2 - PML*ds, 
                              xw/2 + PML*ds, 
                              self.yres)
        ...
        self.xy = RectMesh2D(xy_xw, xy_yw, ds,
                             ds, PML)
    ...
    def sigmax(self, x):
        ...
        return np.where(np.abs(x)>self.xy.xw/2., 
                        power((np.abs(x) - self.xy.xw/2)/(self.PML*self.xy.dx0),2.)\
                            *self.sigma_max, 
                        0.+0.j)
    ...
    def sigmay(self, y):
        ...
        return np.where(np.abs(y)>self.xy.yw/2.,
                        power((np.abs(y) - self.xy.yw/2)/(self.PML*self.xy.dy0),2.)\
                            *self.sigma_max,
                        0.+0.j)
