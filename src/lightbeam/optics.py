...
# some ideas
#       -currently have outer bbox to speed up raster. maybe an inner bbox 
#        will also speed things up? this would force fancy indexing though...

#       -but the boundary region between inner and outer bbox could also be 
#        split into four contiguous blocks

#       -extension to primitives with elliptical cross sections

#       -I don't think this is that hard. An ellipse is a stretched circle. 
#        So antialiasing the ellipse on a rectangular grid is the same as
#        antialiasing a circle on another differently stretched rectangular 
#        grid.
...
class OpticPrim:
    '''
        base class for optical primitives 
        (simple 3D shapes with a single IOR value)
    '''
    ...
    def _bbox(self, z):
        '''
            calculate the 2D bounding box of the primitive at given z. allows 
            for faster IOR computation. Should be overwritten
        '''
    ...
    def _contains(self, x, y, z):
        '''
            given coords, return whether or not those coords are inside the 
    ...
    def bbox_idx(self, z):
        '''
            get index slice corresponding to the primitives bbox, given 
            an xg,yg coord grid
        '''
    ...
    def set_IORsq(self, out, z, 
                  coeff=1):
        ''' 
            replace values of out with IOR^2, given coordinate grids 
            xg, yg, and z location. 
        ...
        if self.z_invariant and self.mask_saved is not None:
            ...
            mask = self._contains(self.xymesh.xg[bbox], 
                                  self.xymesh.yg[bbox], 
                                  z)
    ...
    def get_boundary(self, z):
        ''' 
            given z, get mask which will select pixels that lie on top of 
        ...
        mask_r = (NOT ( AND(AND(maskh[1:,1:] == maskh[:-1,1:], 
                                maskh[1:,1:] == maskh[1:,:-1]),
                                maskh[:-1,:-1]==maskh[:-1,1:])))
...
class scaled_cyl(OpticPrim):
    ...
    def __init__(self, xy, r, 
        ...
        ''' 
            Initialize a scaled cylinder, where the cross-sectional geometry 
            along the object's length is a scaled version of the initial 
            geometry. 
            ...
            scale_func -- optional custom function. Should take in z and 
                          return a scale value. 
                          set to None to use a linear scale function, where 
                          the scale factor of the back end is set by ...
    ...
    def set_IORsq(self, out, z,
                  coeff=1):
        '''
            overwrite base function to incorporate anti-aliasing and improve 
            convergence
        '''
        ...
        # *************************************************************
        # def enter2(self):
        #     if np.shape(xhg) != np.shape(yhg):
        #         print('bboxh:', bboxh)
        #         print('shape xhg PRE:', np.shape(self.xymesh.xhg))
        #         print('shape yhg PRE:', np.shape(self.xymesh.yhg))
        #         print('shape xhg POST:', np.shape(xhg))
        #         print('shape yhg POST:', np.shape(yhg))
        #         print('ERORRR!!!')     
        # enter2(self)
        ...
        geom.AA_circle_nonu(out, xg, yg,
                            xhg, yhg, center,
                            self.r*scale, self.nb2*coeff, self.n2*coeff,
                            bbox, rxg, ryg,
                            dxg, dyg)
...
class OpticSys(OpticPrim):
    '''
        base class for optical systems, collections of primitives immersed 
        in some background medium
    '''
    ...
    ##########################################################################
    def set_IORsq(self, out, z, 
                  xg=None, yg=None, coeff=1):
        '''
            replace values of out with IOR^2, given coordinate grids xg, yg, 
            and z location
        '''
...
class lant5(OpticSys):
    ...
    def __init__(self, rcore, rclad,
                 rjack, ncore, nclad,
                 njack, offset0, z_ex,
                 scale_func=None, final_scale=1, nb=1):
        
        ## Cores
        core0 = scaled_cyl([0,0], rcore, z_ex,
                           ncore, nclad, scale_func=scale_func,
                           final_scale=final_scale)
        core1 = scaled_cyl([offset0,0], rcore, z_ex,
                           ncore, nclad, scale_func=scale_func,
                           final_scale=final_scale)
        core2 = scaled_cyl([0,offset0], rcore, z_ex,
                           ncore, nclad, scale_func=scale_func,
                           final_scale=final_scale)
        core3 = scaled_cyl([-offset0,0], rcore, z_ex,
                           ncore, nclad, scale_func=scale_func,
                           final_scale=final_scale)
        core4 = scaled_cyl([0,-offset0], rcore, z_ex,
                           ncore, nclad, scale_func=scale_func,
                           final_scale=final_scale)
        
        ## Cladding
        clad = scaled_cyl([0,0], rclad, z_ex, 
                          nclad, njack, scale_func=scale_func,
                          final_scale=final_scale)
        ## Jacket
        jack = scaled_cyl([0,0], rjack, z_ex, 
                          njack, nb, scale_func=scale_func,
                          final_scale=final_scale)
        
        ## Combined Elements
        elmnts = [jack, clad, core4,
                  core3, core2, core1,
                  core0]
...
class lant5big(OpticSys):
    '''
        corrigan et al. 2018 style photonic lantern except the jacket is 
        infinite
        '''

    def __init__(self, rcore, rclad,
                 ncore, nclad, njack,
                 offset0, z_ex, scale_func=None,
                 final_scale=1):
...
class lant3big(OpticSys):
    ...
    def __init__(self, rcore, rclad,
        ...
        ## Cores
        core0 = scaled_cyl([0,offset0], rcore, z_ex,
                           ncore, nclad, z_offset,
                           scale_func=scale_func, final_scale=final_scale)
        core1 = scaled_cyl([-np.sqrt(3)/2*offset0,-offset0/2], rcore, z_ex,
                           ncore, nclad, z_offset,
                           scale_func=scale_func, final_scale=final_scale)
        core2 = scaled_cyl([np.sqrt(3)/2*offset0,-offset0/2], rcore, z_ex,
                           ncore, nclad, z_offset,
                           scale_func=scale_func, final_scale=final_scale)
        ...
        ## Combined elements
        elmnts = [clad, core2, core1,
                  core0]
        
        super().__init__(elmnts, njack)

        self.init_core_locs = np.array([[0, offset0],
                                        [-np.sqrt(3)/2*offset0, -offset0/2],
                                        [np.sqrt(3)/2*offset0, -offset0/2]])
...
class lant3_ms(OpticSys):
    ...
    def __init__(self, rcore1, rcore2,
        ...
        ## Fundamental Core
        core0 = scaled_cyl([0,offset0], rcore1, z_ex,
                           ncore, nclad, z_offset,
                           scale_func=scale_func, final_scale=final_scale)
        
        ## AO Cores
        core1 = scaled_cyl([-np.sqrt(3)/2*offset0,-offset0/2], rcore2, z_ex,
                           ncore, nclad, z_offset,
                           scale_func=scale_func, final_scale=final_scale)
        core2 = scaled_cyl([np.sqrt(3)/2*offset0,-offset0/2], rcore2, z_ex,
                           ncore, nclad, z_offset,
                           scale_func=scale_func, final_scale=final_scale)
...
class lant6_saval(OpticSys):
    ...
    def __init__(self, rcore0, rcore1,
                 rcore2, rcore3, rclad,
                 ncore, nclad, njack,
                 offset0, z_ex, z_offset=0,
                 scale_func=None, final_scale=1):
        ...
        ## Mode-Selective Cores
        core0 = scaled_cyl(core_locs[0], rcore0, z_ex,
                           ncore, nclad, z_offset,
                           scale_func=scale_func, final_scale=final_scale)
        core1 = scaled_cyl(core_locs[1], rcore1, z_ex,
                           ncore, nclad, z_offset, 
                           scale_func=scale_func, final_scale=final_scale)
        core2 = scaled_cyl(core_locs[2], rcore1, z_ex,
                           ncore, nclad, z_offset,
                           scale_func=scale_func, final_scale=final_scale)
        core3 = scaled_cyl(core_locs[3], rcore2, z_ex,
                           ncore, nclad, z_offset,
        ...
        core4 = scaled_cyl(core_locs[4], rcore2, z_ex,
                           ncore, nclad, z_offset,
                           scale_func=scale_func, final_scale=final_scale)
        core5 = scaled_cyl(core_locs[5], rcore3, z_ex,
                           ncore, nclad, z_offset,
                           scale_func=scale_func, final_scale=final_scale)
