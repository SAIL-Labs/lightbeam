...
##############################################################################
def get_MFD_from_NA(k0, rcore, ncore,
                    NA):

    nclad = np.sqrt(ncore*ncore - NA*NA)

    return get_MFD(k0, rcore, ncore, 
                   nclad)
...
##############################################################################
def findBetween(solve_fn, lowbound, highbound, 
                args=(), maxj=10):
...
def get_b(l, m, V):
    ...
    b_opt = findBetween(solve_fn, lowbound, highbound, 
                        maxj=10, args=(V,))
...
def lpfield_fixed(xg, yg, l, 
    ...
    if l==3 and m==2 and which=='sin':
        if which == 'cos':
            fieldout *= np.cos(l*phis+np.pi/6)
            # fieldout *= np.cos(l*phis)

        else:
            fieldout *= np.sin(l*phis+np.pi/6)
            # fieldout *= np.sin(l*phis)
...
def get_IOR(wl):
    ...
    return np.sqrt(0.6961663 * wl2 / (wl2 - 0.0684043**2) \
                   + 0.4079426 * wl2 / (wl2 - 0.1162414**2) \
                    + 0.8974794 * wl2 / (wl2 - 9.896161**2) + 1)


##############################################################################
def get_num_modes(k0, rcore, ncore,
                  nclad):
...
def get_all_bs(l, V,bmax):
    ...
    def solve_fn(b, V):
        ...
        if l == 0:
            return np.abs(u * jv(1, u) * kv(0, v) - v * jv(0, u) \
                          * kv(1, v))
        
        else:
            return np.abs(u * jv(l - 1, u) * kv(l, v) + v * jv(l, u) \
                          * kv(l - 1, v))
...
if __name__ == "__main__":
    ...
    for mode in modes:
        ...
        if mode[0]==0:
            ...
            print(mode)
            lp= lpfield(xg, yg, mode[0],
                        mode[1], rcore, 1,
                        ncore, nclad, 'cos')
            
            lp /= np.max(lp)

            f = hc.Field(lp.flatten(), hc.make_pupil_grid((1000,1000), 
                                                          diameter=30) )
