...
def _arc(x, y0, y1, r):
    ...
    thetas[~mask] = np.arctan(y1[~mask]/x[~mask]) \
        - np.arctan(y0[~mask]/x[~mask])
...
##############################################################################
def nonu_pixwt(xc, yc, r, 
               x, y, rx,
               ry, dx, dy):

    area = dx*dy/4 * (rx+1)*(ry+1)
    
    return _intarea(xc, yc, r,
                    x-0.5*dx, 
                    x+0.5*rx*dx, 
                    y-0.5*dy, 
                    y+0.5*ry*dy)/area
...
##############################################################################
def AA_circle_nonu(out, xg, yg,
                   xgh, ygh, center,
                   R, n0, n1,
                   where, rxg, ryg,
                   dxg, dyg):

    # print('shape xgh: ', np.shape(xgh))
    # print('shape ygh: ', np.shape(ygh))
    ...
    area = nonu_pixwt(center[0], center[1], R,
                      x0, y0, rx,
                      ry, dx, dy)
...
def AA_circle(out, xg, yg,
    ...
    area = pixwt(center[0]/ds, center[1]/ds, R/ds, 
                 x0/ds, y0/ds)
