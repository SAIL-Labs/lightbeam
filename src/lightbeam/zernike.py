...
def Z_rad(n, m):
    ...
    coeffs = np.power(-1,indices) * factorial(n-indices) / factorial(indices) \
        / factorial( int((n+m)/2) - indices ) \
            / factorial( int((n-m)/2) - indices)
    powers = n - 2 * indices

    def _inner_(r):
        return np.sum( coeffs[:,None,None] \
                      * np.power( np.repeat([r], len(powers), axis=0), 
                                 powers[:,None,None] ), axis = 0 )
...
def inner_product(u0, u1, ds):
    """inner product which makes use of orthogonality relation of Zernikes 
